...
{
    public class ModelSubastaResumenMes
    {
		#region Constructors
        public ModelSubastaResumenMes()
        {
			CreateBase();
		...
		public ModelSubastaResumenMes(int anio, int mes, int senaladas, int suspendidas)
		{
			CreateBase();
            Anio = anio;
            Mes = mes;
		    ...
		    SubastasCelebradas = SubastasSenaladas - SubastasSuspendidas;
        }
        public ModelSubastaResumenMes(int anio, int mes, int senaladas, int suspendidas, int celebradas)
        {
            CreateBase();
            Anio = anio;
            Mes = mes;
		...
		public decimal SubastasCelebradasPorciento
		{
			get
			{
				if (!_subastasCelebradasPorciento.HasValue)
					_subastasCelebradasPorciento = SubastasSenaladas == 0 ? 0 : 100M * SubastasCelebradas / SubastasSenaladas;
